...
mkdir -p /boot/efi/boot-new

if [ -f "$(echo /boot/system-boot*.efi)" ]; then
	update_pcr4_policy=true
	efi_name="$(echo /boot/system-boot*.efi | sed -n "s@/boot/system-@@p")"
	mv /boot/system-boot*.efi /boot/efi/boot-new/"$efi_name"
else
	efi_name="$(echo /boot/efi/boot/boot*.efi | sed -n "s@/boot/efi/boot/@@p")"
...
if [ -f /boot/vmlinuz ]; then
	...
	# https://manpages.debian.org/trixie/initramfs-tools-core/mkinitramfs.8.en.html
	...
	mv /boot/vmlinuz /boot/efi/boot-new/
else
